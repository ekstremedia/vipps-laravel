 ...
 * `recurring.agreement-expired.v1`: the customer never approved the pending
 * agreement before its deadline, so it expired without ever activating. The
 * payload's `agreementId` names the agreement.
 *
 * Same listener discipline as the generic event: queue, persist, dedupe.
 */
final readonly class AgreementExpired
{
    /**
